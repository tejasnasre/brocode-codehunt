...
import { Building, Mail, MapPin, Briefcase, Globe, Edit, Plus, Search, FileText } from "lucide-react"
...
export default function RecruiterDashboard() {
  ...
  const { role, user } = useAuthStore() as { role: string; user: { id: string } | null }
  ...
  useEffect(() => {
    ...
    const fetchRecruiterData = async () => {
      setLoading(true)
      const { data, error } = await supabase.from("recruiters").select("*").eq("id", user?.id).single()
  ...
  if (loading) {
    return (
      <div className="max-w-6xl mx-auto p-6 bg-indigo-200">
        <h1 className="text-4xl font-bold mb-6 text-black">Recruiter Dashboard</h1>
    ...
    return (
      <div className="max-w-6xl mx-auto p-6 bg-indigo-200 text-center">
        ...
        <div className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-8">
          <h2 className="text-2xl font-bold mb-4 text-black">No Profile Data Found</h2>
          <p className="text-gray-700 mb-6">We couldn&apos;t find your recruiter profile information.</p>
          <button
  ...
  const profileItems = [
    { icon: <Building className="h-6 w-6" />, title: "Company", value: recruiterData.company },
    { icon: <Mail className="h-6 w-6" />, title: "Email", value: recruiterData.email },
    { icon: <MapPin className="h-6 w-6" />, title: "Location", value: recruiterData.location || "Not provided" },
    {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-indigo-200 my-10">
      ...
      <div className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-6 mb-8">
        <h2 className="text-3xl font-bold mb-4 text-black">{recruiterData.full_name}</h2>
        <p className="text-xl text-gray-700">
          Welcome to your recruiter dashboard. Here you can manage your profile and view your recruitment activities.
        </p>
      </div>

      <h2 className="text-2xl font-bold mb-4 text-black">Profile Information</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {profileItems.map((item, index) => (
          <div key={index} className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-4">
